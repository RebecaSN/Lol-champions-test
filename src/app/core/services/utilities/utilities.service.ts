...
export class UtilitiesService {
  ...
  public static readonly noNetworkMessageCaption = 'network-messages.title.no-network'; // 'No Network';
  public static readonly noNetworkMessageDetail = 'network-messages.detail.no-network'; // 'The server cannot be reached';
  public static readonly accessDeniedMessageCaption = 'network-messages.title.access-denied'; // 'Access Denied!';
  public static readonly accessDeniedMessageDetail = 'network-messages.detail.access-denied';
  public static readonly notFoundMessageCaption = 'network-messages.title.not-found'; // 'Not Found';
  public static readonly notFoundMessageDetail = 'network-messages.detail.not-found'; // 'The target resource cannot be found';
  public static readonly sectionExpiredTitle = 'section.expired.title';
  public static readonly sectionExpiredDetail = 'section.expired.detail';
  public static readonly internalServerErrorTitle = 'network-messages.title.internal-server-error';
  ...
  public static getHttpResponseMessages(data: HttpResponseBase | any): string[] {
    ...
    if (this.checkTokenExpired(data)) {
    ...
    if (this.checkInternalServerError(data)) {
      let message = `${this.internalServerErrorTitle}`;
      if (data.url) {
        message += `. ${data.url}`;
      }
  ...
  public static getHttpResponseMessage(data: HttpResponseBase | any): string {
    const httpMessage =
      UtilitiesService.findHttpResponseMessage(UtilitiesService.noNetworkMessageCaption, data) ||
      UtilitiesService.findHttpResponseMessage(UtilitiesService.notFoundMessageCaption, data) ||
      UtilitiesService.findHttpResponseMessage('error_description', data) ||
      UtilitiesService.findHttpResponseMessage('error', data) ||
      UtilitiesService.getHttpResponseMessages(data).join();
  ...
  public static findHttpResponseMessage(
    ...
    for (const message of httpMessages) {
      const fullMessage = UtilitiesService.splitInTwo(message, this.captionAndMessageSeparator);
  ...
  public static checkNoNetwork(response: HttpResponseBase): boolean {
    ...
    // console.log(response instanceof HttpResponseBase)
  ...
  public static checkTokenExpired(response: HttpResponseBase): boolean {
    if (response instanceof HttpResponseBase) {
  ...
  public static checkInternalServerError(response: HttpResponseBase): boolean {
    if (response instanceof HttpResponseBase) {
  ...
  public static getQueryParamsFromString(paramString: string): { [key: string]: string; } {
    ...
    for (const param of paramString.split('&')) {
      const keyValue = UtilitiesService.splitInTwo(param, '=');
  ...
  public static safeStringify(object): string {
  ...
  public static TestIsObjectEmpty(obj: any): boolean {
  ...
  public static printDate(date: Date, separator = 'at'): string {
    return `${UtilitiesService.printDateOnly(date)} ${separator} ${UtilitiesService.printTimeOnly(date)}`;
  }
  ...
  public static printFriendlyDate(date: Date, separator = '-'): string {
    ...
    if (test.toDateString() === today.toDateString()) {
      return `Today ${separator} ${UtilitiesService.printTimeOnly(date)}`;
    }
    if (test.toDateString() === yesterday.toDateString()) {
      return `Yesterday ${separator} ${UtilitiesService.printTimeOnly(date)}`;
    } else {
      return UtilitiesService.printDate(date, separator);
    }
  }
  ...
  public static printShortDate(date: Date, separator = '/', dateTimeSeparator = '-'): string {
    ...
    return `${month}${separator}${day}${separator}${year} ${dateTimeSeparator} ${UtilitiesService.printTimeOnly(date)}`;
  }
  ...
  public static getAge(birthDate, otherDate): number {
  ...
  public static moveArrayItem(array: any[], oldIndex, newIndex): void {
    if (oldIndex < 0) {
      return;
    }
  ...
  public static convertToAbsoluteUrl(url: string): string {
    ...
    return UtilitiesService.testIsAbsoluteUrl(url) ? url : '//' + url;
  }
  ...
  public static removeNulls(obj): any {
    ...
    for (const k in obj) {
      ...
      if (obj[k] === null) {
        ...
        UtilitiesService.removeNulls(obj[k]);
      }

      if (isArray && obj.length === +k) {
        UtilitiesService.removeNulls(obj);
      }
    }
    return obj;
  }

  public static debounce(func: (...args) => any, wait: number, immediate?: boolean): () => void {
    let timeout;
  ...
  public static isUndefined(value: any): any {
    return value === undefined ? null : value;
  }

  public static isNullOrUndefinedToBlank(value: any): any {
    return value === undefined || value === null ? '' : value;
  }

  public static isNullOrUndefinedToZero(value: number): any {
    return value === undefined || value === null ? 0 : value;
  }

  public static isObjectUndefined(obj: any): any {
    return obj === undefined ? null : obj;
  }

  public static isNullOrUndefinedCheck(value: any): boolean {
